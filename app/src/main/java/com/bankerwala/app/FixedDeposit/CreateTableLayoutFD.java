 ...
 */
public class CreateTableLayoutFD extends Fragment implements Cloneable {
    ...
    public String calculateMaturity(String principal, String DurationType) {
        ...
        Double maturity = MathCalculations.CalculateFixedDeposit(Double.parseDouble(principal.replace(",", "")), Double.parseDouble(interest), 1d, DurationType, CompoundingType);
    ...
    public String calculateMaturity(String principal, String DurationType, String Duration) {
        ...
        Double maturity = MathCalculations.CalculateFixedDeposit(Double.parseDouble(principal.replace(",", "")), Double.parseDouble(interest), Double.parseDouble(Duration), DurationType, CompoundingType);
        Log.d(taglaunch, "Creating Table Calculated MaturityValue " + maturity);
        return formatValue(maturity);
    }
    ...
    public void createTableForMonthView() {
        ...
        String initialMaturityValue = principal;
        ...
        for (int i = 0; i < duration; i++) {
            ...
            String LoopMaturityValue = calculateMaturity(initialMaturityValue, "Month(s)");
    ...
    public void createTableForYearView() {
        ...
        initialInterestEarned = interestEarned(initialMaturityValue, formatValue(Double.parseDouble(principal)));
        ...
        if (spin > 1)
            for (i = 1; i <= spin; i++) {
                ...
                LoopMaturityValue = calculateMaturity(initialMaturityValue, "Year(s)");
                tbrow.addView(getTextView(interestEarned(LoopMaturityValue, initialMaturityValue), textView3.getMeasuredWidthAndState(), 0.70f, i));
        ...
        if (duration != 0) {
            ...
            LoopMaturityValue = calculateMaturity(initialMaturityValue, "Month(s)", "" + duration);
    ...
    public void createTable(String DurationTypeFromRadio) {

        Log.d(TableCreation, "Table Creation in Progress");

        Log.d(taglaunch, "Memory Stk " + stk.toString());

        if (DurationTypeFromRadio.equals("Month(s)")) {

            if (periodType.equals("Year(s)")) duration = Integer.parseInt(period) * 12;
            else duration = Integer.parseInt(period);

            additionalCalulcation = false;

        } else if (DurationTypeFromRadio.equals("Year(s)")) {

            if (Integer.parseInt(period) < 12 && periodType.equals("Month(s)")) {
                createTable("Month(s)", true);
                return;
            }

            if (periodType.equals("Month(s)")) duration = Integer.parseInt(period) / 12;
            else duration = Integer.parseInt(period);

            if (periodType.equals("Month(s)"))
                if (Integer.parseInt(period) % 12 != 0) additionalCalulcation = true;
        }
        ...
        String initialMaturityValue = calculateMaturity(principal, DurationTypeFromRadio);
        ...
        for (i = 1; i < duration; i++) {
            ...
            LoopMaturityValue = calculateMaturity(initialMaturityValue, DurationTypeFromRadio);
        ...
        if (additionalCalulcation) {


            if (DurationTypeFromRadio.equals("Year(s)")) {
                tbrow = new TableRow(getActivity());

                tbrow.addView(getTextView(getYear(i), textView1.getMeasuredWidthAndState(), 0.5f, i));

                String extraPeriod = "" + Integer.parseInt(period) % 12;


                LoopMaturityValue = calculateMaturity(initialMaturityValue, "Month(s)", extraPeriod);
                tbrow.addView(getTextView(initialMaturityValue, textView2.getMeasuredWidthAndState(), 0.90f, i));
                tbrow.addView(getTextView(interestEarned(LoopMaturityValue, initialMaturityValue), textView3.getMeasuredWidthAndState(), 0.70f, i));
                tbrow.addView(getTextView(LoopMaturityValue, textView4.getMeasuredWidthAndState(), 0.90f, i));
                stk.addView(tbrow);
            ...
            if (DurationTypeFromRadio.equals("Month(s)")) {
                ...
                String extraPeriod = "" + Integer.parseInt(period) % 30;


                LoopMaturityValue = calculateMaturity(initialMaturityValue, "Day(s)", extraPeriod);
                tbrow.addView(getTextView(initialMaturityValue, textView2.getMeasuredWidthAndState(), 0.90f, i));
                tbrow.addView(getTextView(interestEarned(LoopMaturityValue, initialMaturityValue), textView3.getMeasuredWidthAndState(), 0.70f, i));
                tbrow.addView(getTextView(LoopMaturityValue, textView4.getMeasuredWidthAndState(), 0.90f, i));
                stk.addView(tbrow);
    ...
    public void createTable(String periodType, Boolean status) {
        stk.removeAllViews();
        TableRow tbrow = new TableRow(getActivity());
        ...
        String initialMaturityValue = calculateMaturity(principal, periodType, period);
        String initialInterestEarned = interestEarned(initialMaturityValue, principal);
